...
async def afwezig(message):
    ...
    if isinstance(message.channel, discord.channel.DMChannel):
        ...
        while not is_valid_use_hours:
            ...
            use_hours = await bot.wait_for("message", check=user_check, timeout=60)
        ...
        await bot.get_channel(int(env.private_review_channel_id)).send(embed=PrivateChannel.pending(absence_request),
                                                                       view=AbsenceRequestView(absence_request,
                                                                                               absence_request_insert.inserted_id))

        # Notify user
        await bot.get_user(int(absence_request.user_id)).send(embed=DirectMessage.pending(absence_request))
